...
function initLensReveal() {
    const hero = document.querySelector<HTMLElement>("[data-lens-hero]");
    if (!hero || hero.dataset.initialized === "true") return;
    hero.dataset.initialized = "true";

    const canvas = hero.querySelector<HTMLCanvasElement>("[data-lens-canvas]");
    const logo = hero.querySelector<HTMLElement>("[data-reveal-logo]");
    const shadow = hero.querySelector<HTMLElement>("[data-lens-shadow]");
    const status = hero.querySelector<HTMLElement>("[data-lens-status]");
    const progress = hero.querySelector<HTMLElement>("[data-lens-progress]");
    ...
    try {
        ...
        hero.classList.add("is-fallback", "is-revealed");
        logo.style.setProperty("--logo-reveal", "100%");
        if (status) status.textContent = "Identidad revelada";
        return;
    }
    ...
    const ambient = new THREE.HemisphereLight(0xffffff, 0xc8c8c8, 2.4);
    scene.add(ambient);
    ...
    const resize = () => {
        ...
        targetSizePx = viewportWidth <= 680
            ? clamp(viewportWidth * 0.52, 184, 228)
            : clamp(viewportWidth * 0.205, 250, 350);
        targetSize = targetSizePx * (viewHeight / viewportHeight);
        wheelRadius = targetSize * 0.46;
        travelEdge = viewWidth / 2 + targetSize * 0.62;

        const screenYRatio = viewportWidth <= 680 ? 0.535 : 0.515;
    ...
    const setLoadingProgress = (loaded: number, total: number) => {
        ...
        if (status) status.textContent = total > 0
            ? `Preparando óptica · ${Math.round(value * 100)}%`
            : "Preparando óptica";
    };

    const finishLoading = () => {
        hero.classList.add("is-model-ready");
        if (status) status.textContent = "Óptica lista";
        if (progress) progress.style.transform = "scaleX(1)";
        animationStart = performance.now() + 220;
    };
    ...
    const finishLens = () => {
        if (lensFinished) return;
        lensFinished = true;
        hero.classList.add("is-lens-finished");
    ...
    const updateLogoReveal = (worldX: number) => {
        ...
        const discoveryEdge = screenX + targetSizePx * 0.08;
        const revealed = clamp((discoveryEdge - logoRect.left) / logoRect.width);
        revealMaximum = Math.max(revealMaximum, revealed);
        logo.style.setProperty("--logo-reveal", `${revealMaximum * 100}%`);
    ...
    const renderFrame = (time: number) => {
        ...
        if (reducedMotion.matches) {
            const staticX = viewWidth * 0.27;
            rollingGroup.position.x = staticX;
            ...
            const revealDuration = viewportWidth <= 680 ? 3500 : 4400;
            let worldX: number;
            let rotation: number;

            if (elapsed <= revealDuration) {
                const progressValue = easeInOutCubic(clamp(elapsed / revealDuration));
                worldX = THREE.MathUtils.lerp(-travelEdge, travelEdge, progressValue);
                rotation = -(worldX + travelEdge) / wheelRadius;
                updateLogoReveal(worldX);
            } else {
                const loopDuration = viewportWidth <= 680 ? 7000 : 8400;
                const phase = ((elapsed - revealDuration) % loopDuration) / loopDuration;
                worldX = travelEdge * Math.cos(phase * Math.PI * 2);
                rotation = -(travelEdge * 2) / wheelRadius - (worldX - travelEdge) / wheelRadius;
                if (revealMaximum < 1) {
                    revealMaximum = 1;
                    logo.style.setProperty("--logo-reveal", "100%");
                    hero.classList.add("is-revealed");
    ...
    const handleVisibility = () => {
        if (!document.hidden && modelVisual) {
...
if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initLensReveal, { once: true });
} else {
    initLensReveal();
}
